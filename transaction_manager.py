...
class TransactionManager:
    ...
    def deposit(self, ac_no):
        ...
        self.update_transaction_details(ac_no,transaction_id,amount,transaction_type, before_balance)
    ...
    def withdraw(self,ac_no):
        ...
        try:
            ...
            if before_balance - amount < 1000:
                print("Insufficient balance. Minimum balance must be 1000 after withdrawal.")
                return None
            new_balance = before_balance - amount
            cursor.execute(
                """
    ...
    def send_money(self,ac_no):
        ...
        if conn is None or cursor is None:
            return
        try:
            cursor.execute("SELECT balance FROM account_details WHERE ac_no = %s", (ac_no,))
            sender_balance_result = cursor.fetchone()
            cursor.execute("SELECT balance FROM account_details WHERE ac_no = %s", (receiver_ac_no,))
            receiver_balance_result = cursor.fetchone()
            if sender_balance_result and receiver_balance_result and sender_balance_result[0] - amount >= 1000:
                transaction_id = self.ti.generate_transaction_id()
                ...
                cursor.execute("INSERT INTO send_money (transaction_id, sender_account_no, receiver_account_no, amount_sent, balance) VALUES (%s, %s, %s, %s, %s)", (transaction_id, ac_no, receiver_ac_no, amount, new_sender_balance))
                conn.commit()
                self.update_account_details(ac_no,-(amount))
                self.update_account_details(receiver_ac_no,amount)
                transaction_type = "Send Money"
                rec_transaction_type = "Deposit"
                self.update_transaction_details(ac_no,transaction_id,amount,transaction_type, sender_before_balance)
                self.update_transaction_details(receiver_ac_no,transaction_id,amount,rec_transaction_type, receiver_before_balance)
                print(f"Sent {amount} to {receiver_ac_no}. New balance: {new_sender_balance}")
                return True
            else:
                print("Insufficient balance. Minimum balance must be 1000 after sending, or invalid receiver.")
                return False
        except Exception as e:
            print(f"Error: {e}")
            conn.rollback()
    ...
    def update_transaction_details(self, ac_no, transaction_id, amount, transaction_type, before_balance):
        conn, cursor = self.connection()
        if conn is None or cursor is None:
            return None
        try:
            ...
            cursor.execute(
                ...
                (transaction_id, ac_no, transaction_type, credit, debit, before_balance, after_balance)
            )
            conn.commit()
        except Exception as e:
            print(f"Database Error: {e}")
    ...
    def activate_account(self,ac_no):
        print()
        print()
        print("!!! Your Account is not active please Deposit Rs.1000 minimum to Activate your account")
        self.deposit(ac_no)
        conn , cursor = self.connection()
        if conn is None or cursor is None:
            return None
    ...
    def get_transaction_history(self, ac_no):
        ...
        if conn is None or cursor is None:
            return None
        try:
            cursor.execute("""SELECT id, transaction_id, account_no, transaction_type, credit, debit, before_balance, after_balance, created_at 
            ...
            for row in results:
                to_ac_no = None
                if row[3] == 'Send Money':
                    cursor.execute("SELECT receiver_account_no FROM send_money WHERE transaction_id = %s", (row[1],))  # transaction_id
    ...
    def temp_update_account_details(self, ac_no):
        ...
        if conn is None or cursor is None:
            return None
        try:
            cursor.execute("SELECT ac_no FROM account_details WHERE ac_no = %s", (ac_no,))
            if cursor.fetchone():
